...
class ComplaintService:
    ...
    async def create_complaint(
        ...
        created = await self.repository.create(complaint, triage_result=triage_result, session=session)

        # Invalidate Redis Stats Cache on write
        await self.stats_service.invalidate_stats_cache()
        return created

    async def get_complaint(self, complaint_id: str, session: AsyncSession | None = None) -> ComplaintResponse | None:
